...
## Step 2-3: Exposures’ correlations & Outcomes’ associations

After completing step 1, proceed to upload the exposures and covariate data in step 2 of the TriplotGUI interface. Upon briefly inspecting the variable classes, you'll notice some important details:
...
::: callout-note
-   Select *One-hot-encoding* in ***Categorical variables*** transform categorical exposure variables (like `"HFI"`) with more than two classes into multiple binary variables. This process creates n binary variables for a categorical variable with n classes. The correlation coefficients between modules and these new binary variables are then calculated.
