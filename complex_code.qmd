...
The modules information saved in the TPObject can be directly utilized in the `makeCorr()` function. This function calculates correlation coefficients and p-values between the modules and exposures (food variables and diet scores), while adjusting for specified confounders.
...
::: callout-note
-   The `makeCorr()` function offers options for handling categorical variables and adjusting for confounders. When `allowcategorical=F`, categorical exposures with more than two classes undergo one-hot-encoding, transforming them into binary variables. This can be verified by examining `Correlations_object$cor_estimate`, which shows separate correlation coefficients for each binary representation of HFI levels.

    Conversely, setting `allowcategorical=T` leaves categorical variables like `"HFI"` untransformed. In this case, linear models regressing modules on `"HFI"` are used for correlation analysis. The correlation coefficients are derived from the square root of the proportion of HFI's sum of squares to the total sum of squares.

-   Setting `partial=T` ensures that specified confounders are adjusted for in the correlation analysis.

:::

The correlation results obtained from the `makeCorr()` function are then incorporated into the TPObject. 
...
The modules information saved in the TPObject can be directly utilized in the `coefficient_get()` function. This function calculates risk estimates and p-values between the modules and outcomes (BMI, type 2 diabetes and categorical BMI), while adjusting for specified confounders.
...
The `coefficient_get()` function offers various options for handling outcomes and adjusting for confounders, with behavior differing based on whether pair information is provided.

-   Without `pair` information:
    -   `multinomial=F` transforms categorical outcomes with >2 classes into binary variables via one-hot-encoding. Logistic regression is used for binary outcomes, and linear regression for continuous outcomes.
    -    `multinomial=T` applies multinomial regression to categorical outcomes with >2 classes, yielding n-1 estimates. Binary and continuous outcomes are handled as before.
    
-   With pair information:
    -   `multinomial=F` transforms categorical outcomes as above, then uses conditional logistic regression for binary outcomes and linear mixed models for continuous outcomes, incorporating pairing information.
    -   `multinomial=T` ignores pairing information, applying multinomial regression to categorical outcomes and standard logistic/linear regression to binary/continuous outcomes.
    
-   Setting `partial=T` ensures adjustment for specified confounders in all analyses.


:::

The results for risk estimates obtained from the `coefficient_get()` function are then incorporated into the TPObject. 
...
::: callout-note
Note that we use first and second module for our example here. In reality, you may use the `checkTPO_ggplot()` function in TriplotGUI to view which modules may be more of your interest (that are associated with specific exposures and outcomes).
:::
...
Add the mediation result from `get_mediation_counterfactual()` into the TPObject.
...
The `mediation_triplot()` function displays only the proportion mediated (PM) and adjusted proportion mediated (APM) as dark green triangle points due to the specified `which_show = c("PM", "APM")` argument.

To reduce visual clutter, you can separate these by running the function twice: once with `which_show = c("PM")` and once with `which_show = c("APM")`. 
...
Specifying `which_show = c("PM", "APM", "IE", "TE")` in the `mediation_triplot()` function would display all estimates, including indirect effect (IE) and total effect (TE). However, this is not recommended as it can overcrowd the plot, making it difficult to interpret. 
...
The `checkTPO_ggplot()` function in TriplotGUI allows users to generate heatmaps from the TPObject to visualize correlations, risk estimations, and mediation results.
